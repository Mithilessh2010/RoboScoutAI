...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		if (url.searchParams.get('eventCode')) queryParams.append('eventCode', url.searchParams.get('eventCode')!);
		if (url.searchParams.get('state')) queryParams.append('state', url.searchParams.get('state')!);
		if (url.searchParams.get('excludeNonCompeting')) queryParams.append('excludeNonCompeting', url.searchParams.get('excludeNonCompeting')!);
		if (url.searchParams.get('page')) queryParams.append('page', url.searchParams.get('page')!);

		const apiUrl = `${baseUrl}/${season}/teams${queryParams.toString() ? '?' + queryParams : ''}`;
