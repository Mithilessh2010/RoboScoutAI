...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		if (url.searchParams.get('tournamentLevel')) queryParams.append('tournamentLevel', url.searchParams.get('tournamentLevel')!);
		...
		const apiUrl = `${baseUrl}/${season}/schedule/${eventCode}${queryParams.toString() ? '?' + queryParams : ''}`;
