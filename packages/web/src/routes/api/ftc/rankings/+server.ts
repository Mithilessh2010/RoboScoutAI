...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		if (url.searchParams.get('top')) queryParams.append('top', url.searchParams.get('top')!);

		const apiUrl = `${baseUrl}/${season}/rankings/${eventCode}${queryParams.toString() ? '?' + queryParams : ''}`;
