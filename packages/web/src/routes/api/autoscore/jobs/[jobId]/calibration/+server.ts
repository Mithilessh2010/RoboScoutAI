...
import { ensureAutoscoreDb, ManualCalibrationZone, jsonResponse } from "$lib/server/autoscore";

export const GET: RequestHandler = async ({ params }) => {
    await ensureAutoscoreDb();
    const zones = await ManualCalibrationZone.find({ jobId: params.jobId }).lean();
