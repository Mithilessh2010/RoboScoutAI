<script lang="ts">
    ...
    async function editEvent(event:any){
        ...
        await api(`/api/autoscore/jobs/${jobId}/timeline/${event._id}`, { method:"PUT", headers:{"content-type":"application/json"}, body:JSON.stringify({ alliance, eventType, points, reason, manualOverride:true, reviewed:true }) });
        await recalc();
    }
...
<button class="delete-event" on:click={()=>editEvent(event)}>Edit</button>
...
<style>
h1,h2,h3,p{margin:0} header{display:flex;justify-content:space-between;margin-bottom:16px}.cockpit{display:grid;grid-template-columns:minmax(0,2fr) 360px;gap:16px}.video-wrap{position:relative;background:#050505;aspect-ratio:16/9}.video-wrap video,.video-wrap canvas{position:absolute;inset:0;width:100%;height:100%}.video-wrap canvas{pointer-events:auto}.toolbar,.scoreboard,.grid,.telemetry{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}.scoreboard article,.telemetry article{flex:1;padding:12px;border:1px solid var(--sep-color);border-radius:8px;display:grid;gap:4px}.scoreboard b{font-size:32px}.red b{color:#ff5c70}.blue b{color:#5fa8ff}.telemetry p{font-size:20px}.telemetry small{color:var(--secondary-text-color)} aside{display:grid;gap:10px} input,select,button{padding:9px;border:1px solid var(--sep-color);border-radius:8px;background:var(--form-bg-color);color:var(--text-color)}button{background:var(--theme-color);color:var(--theme-text-color);cursor:pointer}.secondary{background:var(--form-bg-color);color:var(--text-color)} aside label{display:grid;grid-template-columns:1fr 90px 90px;gap:6px;align-items:center}.zone-list{display:flex;gap:6px;flex-wrap:wrap}.zone-list span{border:1px dashed var(--sep-color);padding:5px;border-radius:6px}.lower{display:grid;grid-template-columns:1.3fr 1fr 1fr;gap:16px;margin-top:18px}.event-row{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:6px;align-items:start;margin-top:6px}.event{display:block;width:100%;text-align:left}.event small{display:block}.event em{margin-left:6px}.delete-event{background:var(--form-bg-color);color:var(--text-color)}dl{display:grid;grid-template-columns:1fr auto;gap:6px;margin:8px 0 16px}.notice,.error{padding:10px;margin-top:12px;border-radius:8px}.notice{background:var(--green-stat-bg-color)}.error{background:var(--red-stat-bg-color)}pre{white-space:pre-wrap;font-size:12px;max-height:360px;overflow:auto}@media(max-width:1000px){.cockpit,.lower{grid-template-columns:1fr}}
</style>
