...
{#if job}
  ...
  <section class="panel">
    ...
    {:else if activeTab === "Detect"}
      <h2>Models & Detection</h2><div class="actions"><button disabled={busy} on:click={() => run(`/api/autoscore/jobs/${jobId}/run-artifact-detection`, "Artifact and robot detection started.")}>Run artifact + robot detection</button><button class="secondary" disabled={busy} on:click={() => run(`/api/autoscore/jobs/${jobId}/run-robot-detection`, "Robot detection started.")}>Run robot detection</button></div>
