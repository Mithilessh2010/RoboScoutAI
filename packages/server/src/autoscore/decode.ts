// @ts-nocheck
import { DECODE_SCORING, decodeMatchSeconds, decodePhaseAt, expectedMotifColor } from "./decode-config";
...
export async function runFullDecodeAutoscore(jobId: string) {
    ...
    await job.updateOne({ status: "scoring", phase: "decode_autoscore", errorMessage: null });

    let detections = await AutoscoreDetection.find({ jobId }).sort({ timestamp: 1, confidence: -1 });
    let zones = await AutoscoreCalibrationZone.find({ jobId }).lean();
    let penalties = await AutoscorePenalty.find({ jobId }).lean();
    let gateEvents = await AutoscoreGateEvent.find({ jobId }).sort({ timestamp: 1 }).lean();
    ...
    if (rampResult.states.length) await AutoscoreRampCountState.insertMany(rampResult.states);
    warnings.push(...rampResult.warnings);

    let events = [
        ...buildLeaveEvents(job),
        ...rampResult.events,
    ...
    let manualEvents = await AutoscoreTimelineEvent.find({ jobId, manualOverride: true }).lean();
    let summary = await calculateAndSaveSummary(jobId, [...events, ...manualEvents], detections, warnings);

    await AutoscoreJob.findByIdAndUpdate(jobId, {
        status: warnings.length ? "review_needed" : "complete",
        phase: "decode_autoscore",
    });
    return { eventsCreated: events.length, summary, rampStates: rampResult.states, warnings };
}
...
export async function recalculateDecodeScore(jobId: string) {
    ...
    return calculateAndSaveSummary(jobId, events, detections, requiredZoneWarnings(zones));
}
...
function buildFrameSnapshots(detections, zones) {
    ...
    for (let detection of detections) {
        ...
        let artifactColor = detection.className === "artifact_green" ? "green" : "purple";
...
function buildRampCountEngine(jobId, frames, gateEvents) {
    ...
    for (let alliance of ["red", "blue"]) {
        ...
        for (let index = 0; index < frames.length; index += 1) {
            ...
            let rampDetections = frame.zones[`ramp_${alliance}`] ?? [];
            ...
            if (countDelta > 0) {
                ...
                for (let n = 0; n < countDelta; n += 1) {
                    events.push(
                        event(jobId, frame.timestamp, decodePhaseAt(frame.timestamp), "classified", alliance, classifiedPoints(frame.timestamp), pathEvidence ? Math.max(0.7, state.confidence) : Math.max(0.45, state.confidence - 0.15), pathEvidence
...
function buildOverflowEvents(jobId, frames, states) {
    ...
    for (let alliance of ["red", "blue"]) {
        ...
        for (let index = 0; index < frames.length; index += 1) {
            let frame = frames[index];
            let square = frame.zones[`square_${alliance}`] ?? [];
            if (!square.length || frame.timestamp - lastOverflowAt < 2) continue;
            let matchingState = states.find((state) => state.alliance === alliance && state.frameNumber === frame.frameNumber);
            let upcomingIncrease = states.some((state) =>
            ...
            let likelyOverflow = (matchingState?.stableCount ?? 0) >= DECODE_SCORING.rampSlots || recentPathEvidence(frames, index, alliance);
            if (!likelyOverflow) continue;
            let detection = square[0];
            rows.push(
                event(jobId, frame.timestamp, decodePhaseAt(frame.timestamp), "overflow", alliance, overflowPoints(frame.timestamp), Math.max(0.45, detection.confidence - 0.12), `${alliance} square activity occurred but ramp stable count did not increase.`, {
...
function buildGateEvents(jobId, gateEvents, states) {
    ...
    for (let gate of gateEvents) {
        rows.push(event(jobId, gate.timestamp, decodePhaseAt(gate.timestamp), "gate_opened", gate.alliance, 0, 1, gate.note || "Manual gate opened event.", { manualOverride: true }));
        let before = [...states].reverse().find((state) => state.alliance === gate.alliance && state.timestamp <= gate.timestamp);
        let after = states.find((state) => state.alliance === gate.alliance && state.timestamp >= gate.timestamp && state.timestamp <= gate.timestamp + 5 && state.countDelta < 0);
        if (before && after) {
            rows.push(event(jobId, after.timestamp, decodePhaseAt(after.timestamp), "ramp_artifacts_released", gate.alliance, 0, 0.9, `${gate.alliance} ramp count decreased ${before.stableCount} -> ${after.stableCount} after manual gate event.`, { manualOverride: true }));
        }
    }
    return rows;
}

function buildLeaveEvents(job) {
    let rows = [];
    for (let key of ["redTeam1", "redTeam2", "blueTeam1", "blueTeam2"]) {
        ...
        rows.push(event(job._id, 30, "AUTO", "leave", alliance, DECODE_SCORING.auto.leavePoints, 1, "Manual LEAVE marked at end of AUTO.", { teamNumber: job[key] || null, manualOverride: true }));
    }
    return rows;
}
...
function buildPatternEvents(job, frames, zones) {
    let rows = [];
    for (let alliance of ["red", "blue"]) {
        let slots = zones.filter((zone) => zone.zoneType === `ramp_index_${alliance}` && zone.index);
        for (let timestamp of [30, decodeMatchSeconds()]) {
            ...
            for (let slot of slots) {
                ...
                let detected = frame.detections.find((d) => pointInPolygon(d.center, slot.coordinates));
                if (!expected || !detected || detected.artifactColor !== expected) continue;
                rows.push(event(job._id, timestamp, timestamp <= 30 ? "AUTO" : "ENDGAME", "pattern", alliance, timestamp <= 30 ? DECODE_SCORING.auto.patternMatchPoints : DECODE_SCORING.teleop.patternMatchPoints, detected.confidence, `Ramp slot ${slot.index} matched motif color ${expected}.`, {
...
function buildDepotEvents(jobId, frames) {
    ...
    return ["red", "blue"].flatMap((alliance) =>
        (frame.zones[`depot_${alliance}`] ?? []).map((det) =>
            event(jobId, decodeMatchSeconds(), "ENDGAME", "depot", alliance, DECODE_SCORING.teleop.depotArtifactPoints, det.confidence, "Artifact over depot in final stable window.", {
...
function buildBaseEvents(job) {
    let rows = [];
    for (let alliance of ["red", "blue"]) {
        let fullCount = 0;
        for (let slot of [`${alliance}Team1`, `${alliance}Team2`]) {
            let value = job.manualBase?.[slot];
            if (value === "partial") rows.push(event(job._id, decodeMatchSeconds(), "ENDGAME", "base_partial", alliance, DECODE_SCORING.teleop.basePartialPoints, 1, "Manual partial BASE return.", { teamNumber: job[slot] || null, manualOverride: true }));
            if (value === "full") {
                fullCount += 1;
                rows.push(event(job._id, decodeMatchSeconds(), "ENDGAME", "base_full", alliance, DECODE_SCORING.teleop.baseFullPoints, 1, "Manual full BASE return.", { teamNumber: job[slot] || null, manualOverride: true }));
            }
        }
        if (fullCount === 2) rows.push(event(job._id, decodeMatchSeconds(), "ENDGAME", "base_bonus", alliance, DECODE_SCORING.teleop.bothRobotsFullBaseBonus, 1, "Both robots fully returned to BASE.", { manualOverride: true }));
    }
    return rows;
}
...
function buildPenaltyEvents(penalties) {
    return penalties.map((penalty) =>
        event(penalty.jobId, penalty.timestamp ?? decodeMatchSeconds(), penalty.timestamp && penalty.timestamp <= 30 ? "AUTO" : "ENDGAME", "penalty", penalty.creditedAlliance, penalty.points, 1, penalty.note || `${penalty.foulType} foul credited to opponent.`, { manualOverride: true })
    );
}
...
async function calculateAndSaveSummary(jobId, events, detections, warnings) {
    ...
    return AutoscoreSummary.findOneAndUpdate(
        { jobId },
        {
            ...
            winner: redTotal === blueTotal ? "tie" : redTotal > blueTotal ? "red" : "blue",
            totalDetections: detections.length,
            artifactGreenCount: detections.filter((det) => det.className === "artifact_green").length,
            artifactPurpleCount: detections.filter((det) => det.className === "artifact_purple").length,
...
function rankingPoints(bucket, ownTotal, otherTotal) {
    let rp = ownTotal > otherTotal ? DECODE_SCORING.rankingPoints.winRP : ownTotal === otherTotal ? DECODE_SCORING.rankingPoints.tieRP : 0;
    if (bucket.auto + bucket.base >= DECODE_SCORING.rankingPoints.movementRPThreshold) rp += DECODE_SCORING.rankingPoints.movementRP;
    if (bucket.classified + bucket.overflow >= DECODE_SCORING.rankingPoints.goalRPThreshold) rp += DECODE_SCORING.rankingPoints.goalRP;
    if (bucket.pattern >= DECODE_SCORING.rankingPoints.patternRPThreshold) rp += DECODE_SCORING.rankingPoints.patternRP;
    return rp;
}

function classifiedPoints(timestamp) {
    return timestamp <= 30 ? DECODE_SCORING.auto.classifiedArtifactPoints : DECODE_SCORING.teleop.classifiedArtifactPoints;
}
function overflowPoints(timestamp) {
    return timestamp <= 30 ? DECODE_SCORING.auto.overflowArtifactPoints : DECODE_SCORING.teleop.overflowArtifactPoints;
}
function scoreBucket() {
    return { auto: 0, teleop: 0, artifact: 0, classified: 0, overflow: 0, pattern: 0, depot: 0, base: 0, penalties: 0 };
}
function requiredZoneWarnings(zones) {
    let present = new Set(zones.map((zone) => zone.zoneType));
    return REQUIRED_ZONE_TYPES.filter((zone) => !present.has(zone)).map((zone) => `${zone} is not calibrated.`);
}
function recentPathEvidence(frames, index, alliance) {
    return frames.slice(Math.max(0, index - 3), index + 1).some((frame) =>
        (frame.zones[`goal_${alliance}`]?.length ?? 0) > 0 &&
        ((frame.zones[`square_${alliance}`]?.length ?? 0) > 0 || (frame.zones[`classifier_${alliance}`]?.length ?? 0) > 0)
    );
}
function nearbyGate(gates, alliance, timestamp) {
    return gates.find((gate) => gate.alliance === alliance && Math.abs(gate.timestamp - timestamp) <= 5);
}
function nearestFrame(frames, timestamp) {
    return [...frames].sort((a, b) => Math.abs(a.timestamp - timestamp) - Math.abs(b.timestamp - timestamp))[0];
}
function normalizedCenter(det) {
    if (!det.frameWidth || !det.frameHeight) return null;
    return { x: ((det.x ?? 0) + (det.width ?? 0) / 2) / det.frameWidth, y: ((det.y ?? 0) + (det.height ?? 0) / 2) / det.frameHeight };
}
function pointInPolygon(point, polygon = []) {
    ...
    for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
        ...
        let intersects = yi > point.y !== yj > point.y && point.x < ((xj - xi) * (point.y - yi)) / (yj - yi || 0.000001) + xi;
        if (intersects) inside = !inside;
    }
    return inside;
}
function event(jobId, timestamp, phase, eventType, alliance, points, confidence, reason, extras = {}) {
    return { jobId, timestamp, phase, eventType, alliance, points, confidence, reason, relatedDetectionIds: [], manualOverride: false, reviewed: false, ...extras };
}
